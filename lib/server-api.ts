...
export async function getServerPosts({ page = 1, limit = 10 }) {
  ...
  const total = parseInt(
    response.headers.get('X-Total-Count') || posts.length.toString()
  );
...
export async function getServerComments(id: number) {
  const cookieStore = await cookies();
  const token = cookieStore.get('token')?.value;

  const response = await fetch(
    `${process.env.NEXT_PUBLIC_API_URL || 'https://jsonplaceholder.typicode.com'}/comments?postId=${id}`,
    {
      headers: {
        'Content-Type': 'application/json',
        ...(token && { Authorization: `Bearer ${token}` }),
